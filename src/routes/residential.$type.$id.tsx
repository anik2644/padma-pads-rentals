  ...
  return (
    <div className="mx-auto max-w-md py-24 text-center">
      ...
      <Button asChild className="mt-4">
        <Link to="/residential">{t("actions.backToBrowse")}</Link>
      </Button>
    </div>
  );
}
        ...
        actions={
          <>
            <button
              ...
              className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-background/90 backdrop-blur hover:scale-105 transition"
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-muted-foreground">
                  {item.rentLabel === "perSeat" ? "/seat/month" : "/month"}
                </p>
                {item.negotiable && (
          ...
          <div className="rounded-3xl border border-border bg-card p-5 shadow-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("detail.memberSince", { year: item.owner.memberSince })}
                </p>
              </div>
            </div>
            <div className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
              <Clock className="h-3.5 w-3.5" />{" "}
              {t("detail.responds", { time: item.owner.responseTime })}
            </div>
            ...
            {item.availableFrom && (
              <div className="mb-4 flex items-center gap-2 text-sm">
                ...
                <span className="font-semibold">{formatDate(item.availableFrom, lang)}</span>
              </div>
            )}
            <div className="space-y-2">
              <Button
                className="w-full gap-2"
                onClick={() => toast.success(t("detail.ownerNotified"))}
              >
